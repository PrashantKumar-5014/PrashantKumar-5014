<h1 align="center">Hi 👋, I'm Prashant Kumar</h1>
<h3 align="center">🚀 Full Stack Developer | MERN Stack | Building Scalable Web Apps</h3>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?lines=Full+Stack+Developer;MERN+Stack+Expert+in+Progress;Building+Real+World+Projects;Open+to+Opportunities&center=true&width=500&height=45">
</p>

---

## 💫 About Me:
🎓 **Certified Full Stack Developer** — IFDA Institute (Grade A++)  
💻 MERN Stack Developer focused on building **real-world scalable applications**  
🚀 Currently building: **Library Management App & EMS Pro**  
🌱 Learning: **Socket.io, WebRTC & System Design**  
👯 Open to collaborate on **Full Stack & Startup Projects**  
💬 Ask me about: **React, Node.js, MongoDB, APIs, JWT Auth**  
⚡ Fun fact: I debug more than I sleep 🐛  

---

## 🚀 Featured Projects

### 📚 KitabGhar — Library Management System
> A complete full-stack bookstore platform with authentication & admin control

🔹 **Tech:** React, Node.js, Express, MongoDB, Tailwind CSS, JWT  
🔹 **Features:**  
- User Authentication (Login/Register)  
- Admin Panel (Add/Edit/Delete Books)  
- Buy Books Functionality  
- Protected Routes & Secure APIs  

---

### 🏢 EMS Pro — Employee Management System
> Scalable employee system with role-based access & real-world features

🔹 **Tech:** React, Node.js, Express, MongoDB, Tailwind CSS, JWT  
🔹 **Features:**  
- Admin Dashboard  
- Employee CRUD  
- Attendance Tracking  
- Leave Management  
- Payroll System  

---

## 🌐 Connect With Me:
<p align="center">
<a href="https://linkedin.com/in/prashant-kumar-68408331b"><img src="https://img.shields.io/badge/LinkedIn-%230077B5.svg?logo=linkedin&logoColor=white"/></a>
<a href="mailto:prashantkumar4216@gmail.com"><img src="https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white"/></a>
<a href="#"><img src="https://img.shields.io/badge/Portfolio-%23000000.svg?logo=vercel&logoColor=white"/></a>
</p>

---

## 🧠 Core Strengths
✔ Clean & scalable architecture  
✔ REST API design & authentication  
✔ Full project lifecycle (UI → Backend → Deployment)  
✔ Strong debugging & problem-solving skills  

---

## 💻 Tech Stack

### 🚀 Frontend
<p>
<img src="https://skillicons.dev/icons?i=react,js,tailwind,html,css" />
</p>

### ⚙️ Backend
<p>
<img src="https://skillicons.dev/icons?i=nodejs,express,mongodb,mysql" />
</p>

### 🛠 Tools & Deployment
<p>
<img src="https://skillicons.dev/icons?i=git,github,postman,vercel,netlify" />
</p>

### 📚 Currently Learning
<p>
<img src="https://skillicons.dev/icons?i=nextjs,threejs" />
<img src="https://img.shields.io/badge/Socket.io-black?style=for-the-badge&logo=socket.io"/>
</p>

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=PrashantKumar-5014&show_icons=true&theme=radical" />
</p>

<p align="center">
  <img src="https://nirzak-streak-stats.vercel.app/?user=PrashantKumar-5014&theme=radical" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=PrashantKumar-5014&layout=compact&theme=radical" />
</p>

---

## 🏆 Achievements
🏅 Full Stack Developer Certification — IFDA (A++)  
🚀 Built multiple real-world MERN projects  
💡 Hands-on experience in authentication & scalable APIs  

---

## 🎯 Current Focus
🔨 Deploying production-ready applications  
📡 Implementing real-time features (Socket.io)  
🌐 Building personal portfolio website  
💼 Actively seeking **Full Stack Developer opportunities**  

---

## 🐍 Contribution Snake
<p align="center">
  <img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake.svg" />
</p>

---

## 👀 Profile Views
<p align="center">
  <img src="https://visitcount.itsvg.in/api?id=PrashantKumar-5014&icon=0&color=0" />
</p>

---

<h3 align="center">🔥 "Code. Build. Deploy. Repeat." 🚀</h3>
